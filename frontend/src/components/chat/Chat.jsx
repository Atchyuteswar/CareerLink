...
const Chat = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const res = await axios.get("http://100.94.122.76:8000/api/v1/user/getall", { 
    ...
    useEffect(() => {
        if (user) {
            const newSocket = io('http://100.94.122.76:8000', {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            ...
            try {
                const res = await axios.get(`http://100.94.122.76:8000/api/v1/message/all/${selectedUser._id}`, {
                    withCredentials: true
                });
                if(res.data.success){
    ...
    const sendMessageHandler = async () => {
        if(!text.trim()) return;
        try {
            const res = await axios.post(`http://100.94.122.76:8000/api/v1/message/send/${selectedUser._id}`, { message: text }, {
